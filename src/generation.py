...
class OllamaGenerator:
    ...
    def invoke(self, prompt):
        # Conservative byte bound leaves room for chat-template/output tokens.
        # Reject oversized input instead of accepting silent server truncation.
        if len(prompt.encode()) + self.settings.max_tokens + 256 > self.settings.context_tokens:
